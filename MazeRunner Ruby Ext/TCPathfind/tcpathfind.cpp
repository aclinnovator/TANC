        ...
        for (int i=0; i<4; i++) {
            ...
            if (points_available[direction]==POINT_REACHABLE) {
                ...
                if ((status(perpA)==UNEXPLORED) && (classification(perpA)==FLOOR)) {
                    addToQueue(translated);
                    addToQueue(perpA);
                    return perpA;
                } else if ((status(perpB)==UNEXPLORED) &&(classification(perpB)==FLOOR)) {
                    addToQueue(translated);
                    addToQueue(perpB);
                    return perpB;
                }else {
...
Point one_square_closer_to_next_location(Point _curr_pos, Point next_location){
  ...
  //as the car.
  Point vec = ceilVec( getVec(_curr_pos, next_location) );
  
  return addVec( _curr_pos, vec);
}
...
void pathfind_print(void){
    ...
    printf("  ");for(int i=0;i<MAZE_WIDTH;i++){printf("%d",i);}printf("  \n\n");
}
